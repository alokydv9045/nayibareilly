...
export default function SuperAdminDashboard() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await api.get('/admin/superadmin/stats')
      setStats(res.data)
    } catch { /* silent */ }
  }, [])
  ...
  const fetchRealtimeIssues = useCallback(async () => {
    try {
      const res = await api.get('/admin/superadmin/realtime-issues')
      setRealtimeIssues(res.data)
    } catch { /* silent */ }
  }, [])

  const fetchModeratorPerformance = useCallback(async () => {
    try {
      const res = await api.get('/admin/superadmin/moderator-performance')
      setModeratorPerformance(res.data)
    } catch { /* silent */ }
  }, [])
  ...
  const statCards = [
    ...
    { label: 'Database Nodes', value: '3/3', sub: 'Healthy cluster', icon: Database, iconBg: 'bg-violet-500/20', iconColor: 'text-violet-600', trend: 'text-emerald-600' },
    { label: 'API Requests', value: '45.2k', sub: 'Last 24 hours', icon: Server, iconBg: 'bg-amber-500/20', iconColor: 'text-amber-600', trend: 'text-amber-600' },
    { label: 'System Uptime', value: `99.99%`, sub: 'All systems operational', icon: Cpu, iconBg: 'bg-emerald-500/20', iconColor: 'text-emerald-600', trend: 'text-emerald-600' },
  ]

  const navCards = [
    { href: '/superadmin/users', icon: UserCog, label: 'User Management', desc: 'Manage accounts, roles & permissions', value: stats.totalUsers, color: 'from-blue-600/20 to-blue-800/20 border-blue-200 hover:border-blue-400/60' },
    { href: '/superadmin/departments', icon: Building2, label: 'Departments', desc: 'Manage departments and assign heads', value: stats.totalDepartments, color: 'from-violet-600/20 to-violet-800/20 border-violet-200 hover:border-violet-400/60' },
    { href: '/issues', icon: Layers, label: 'Issue Override', desc: 'Full visibility and control over all issues', value: stats.totalIssues, color: 'from-amber-600/20 to-amber-800/20 border-amber-200 hover:border-amber-400/60' },
    { href: '/superadmin/analytics', icon: BarChart3, label: 'Analytics & Reports', desc: 'System-wide analytics and insights', value: null, color: 'from-cyan-600/20 to-cyan-800/20 border-cyan-200 hover:border-cyan-400/60' },
    { href: '/superadmin/audit', icon: Lock, label: 'Security & Audit', desc: 'Audit trails and security monitoring', value: null, color: 'from-red-600/20 to-red-800/20 border-red-200 hover:border-red-400/60' },
    { href: '/superadmin/settings', icon: Settings, label: 'System Settings', desc: 'Configure platform-wide settings', value: null, color: 'from-slate-600/20 to-slate-800/20 border-slate-500/30 hover:border-slate-400/60' },
  ]
  ...
  return (
    <div className="min-h-screen bg-white text-black pb-8">
        ...
        <header className="sticky top-0 z-40 bg-white border-b border-amber-200/60 px-8 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-amber-950">Tech Admin & Systems Dashboard</h1>
          ...
          <div className="flex items-center gap-3">
            <Badge className="bg-emerald-500/20 text-emerald-600 border border-emerald-200 gap-1.5">
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
            {statCards.map((card, i) => {
              ...
              return (
                <Card key={i} className="bg-white border border-amber-200/60 hover:bg-amber-50/50 hover:border-amber-200/60 transition-all group">
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="text-sm font-medium text-amber-800/80 mb-2">{card.label}</p>
                        <p className="text-4xl font-bold text-amber-950">{card.value}</p>
          ...
          {/* Navigation Cards */}
          <div>
            <h2 className="text-lg font-semibold text-amber-800/80 mb-4 flex items-center gap-2">
                  ...
                  <Link key={card.href} href={card.href}>
                    <Card className={`bg-gradient-to-br ${card.color}  border transition-all cursor-pointer hover:scale-[1.02] group`}>
                      <CardContent className="p-6">
                        ...
                        <h3 className="text-base font-semibold text-amber-950 mb-1">{card.label}</h3>
                        <p className="text-sm text-amber-800/80 mb-4">{card.desc}</p>
                        {card.value !== null && (
                          <div className="flex items-center gap-2">
                            <span className="text-2xl font-bold text-amber-950">{card.value}</span>
                            <ArrowUpRight className="h-4 w-4 text-amber-800/80" />
                          </div>
                        )}
                      </CardContent>
                    </Card>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <Card className="bg-white border border-amber-200/60">
              <CardHeader className="pb-4">
                ...
                <CardDescription className="text-amber-800/80 text-sm">Real-time issue processing across the platform</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
                  {!Array.isArray(realtimeIssues) || realtimeIssues.length === 0 ? (
                    <div className="text-center py-10 text-amber-800/80">
                      <Activity className="h-10 w-10 mx-auto mb-3 opacity-40" />
                      <p className="text-sm">No active issues right now</p>
                    </div>
                  ) : (
                    realtimeIssues.map((issue, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-white rounded-xl border border-amber-200/60 hover:bg-amber-50/50 transition-all">
                        <div className="flex-1 min-w-0 mr-3">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs font-mono text-purple-600">#{issue.reportId}</span>
                          </div>
                          <p className="text-sm font-medium text-amber-950 truncate">{issue.title}</p>
                          <p className="text-xs text-amber-800/80 flex items-center gap-1 mt-0.5">
                            <Clock className="h-3 w-3" />
                            {issue.timeline}
                          </p>
                        </div>
                        <Badge className={`shrink-0 border text-xs ${STATUS_COLORS[issue.status] || 'bg-amber-100/50 text-amber-800/80 border-amber-200/60'}`}>
                          {issue.status}
                        </Badge>
                      </div>
                    ))
            ...
            <Card className="bg-white border border-amber-200/60">
              <CardHeader className="pb-4">
                ...
                <CardDescription className="text-amber-800/80 text-sm">Assignment accuracy and review efficiency</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
                  {!Array.isArray(moderatorPerformance) || moderatorPerformance.length === 0 ? (
                    ...
                    moderatorPerformance.map((mod, i) => (
                      <div key={i} className="p-4 bg-white rounded-xl border border-amber-200/60">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 bg-purple-500/20 rounded-full flex items-center justify-center">
                              <span className="text-xs font-bold text-purple-600">{mod.name.charAt(0)}</span>
                            </div>
                            <span className="text-sm font-semibold text-amber-950">{mod.name}</span>
                          </div>
                        ...
                        <div className="grid grid-cols-3 gap-3 text-xs">
                          <div className="bg-white rounded-lg p-2 text-center">
                            <p className="text-amber-800/80 mb-0.5">Reviewed</p>
                            <p className="text-amber-950 font-bold text-base">{mod.reviewedCount}</p>
                          </div>
                          <div className="bg-white rounded-lg p-2 text-center">
                            <p className="text-amber-800/80 mb-0.5">Avg Time</p>
                            <p className="text-amber-950 font-bold text-base">{mod.avgReviewTime}m</p>
                          </div>
                          <div className="bg-white rounded-lg p-2 text-center">
                            <p className="text-amber-800/80 mb-0.5">Accuracy</p>
                            <p className="text-amber-950 font-bold text-base">{mod.accuracy}%</p>
                          </div>
                        </div>
                      </div>
                    ))
          ...
          <Card className="bg-white border border-amber-200/60 font-mono shadow-2xl">
            <CardHeader className="pb-4 border-b border-amber-200/60 flex flex-row items-center justify-between bg-white">
              <div className="flex items-center gap-3">
                <Terminal className="h-5 w-5 text-emerald-600" />
                <CardTitle className="text-amber-950 text-base">Live System Logs</CardTitle>
              ...
              <div className="h-80 overflow-y-auto p-4 space-y-1.5 text-xs">
                {systemLogs.map((log, i) => (
                  <div key={i} className="flex items-start gap-4 hover:bg-white px-2 py-1 rounded transition-colors group">
                    <span className="text-amber-800/80 shrink-0 w-36">
